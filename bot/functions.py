...
async def get_today_orders_by_manager(sales_manager_id):
    ...
    for order in orders["order"]:
        if order["sales_manager_id"] == sales_manager_id:
            # deal_time ni datetime ob'ekti bilan solishtirish
            deal_time = datetime.strptime(order["deal_time"], "%d.%m.%Y %H:%M:%S")
...
async def get_last_day_orders_by_manager(sales_manager_id):
    ...
    filtered_orders = [
        order for order in orders["order"] 
...
# await get_lastday_orders()
# latest_orders = get_today_orders_by_manager("updated_orders.json", "3813963")
