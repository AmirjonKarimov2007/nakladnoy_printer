...
async def get_order_for_balans(begin_date,ended_date):

    uzbekistan_tz = pytz.timezone('Asia/Tashkent')
    begin_date = datetime.now(uzbekistan_tz).strftime('%d.%m.%Y')
    ended_date = datetime.now(uzbekistan_tz).strftime('%d.%m.%Y')
    url = "https://smartup.online/b/trade/txs/tdeal/order$export"
